...
def objective(x, y, beta, l):
    vec = vsum(x=x, y=y_train.reshape((len(y_train), 1)), beta=beta)
    return norm(vec) + l*norm(beta)**2
...
def computegrad(x, y, l, b):
    u = np.diag([(exp(yi * xi @ b) + 1)**-1 for xi, yi in zip(x, y)])
    return 2*l*norm(b)-(x.T @ u @ y)/len(y)


def backtracking(x, beta, l, t=1., eta_t=0.5, alpha=0.5, max_iter=1000):
...
def graddescent(x, y, t_init, l=None, eps=0.001):
    ...
    while norm(grads[-1]) > eps:
        t = backtracking(x=xvals[-1], beta=betas[-1], l=l, t=t_init)
...
t_init = 34 #calc_t_init(X_train, .01)
fx, bt = graddescent(X_train, y_train, t_init)
